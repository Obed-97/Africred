            ...
            <div class="card">
                <div class="card-body">
                        ...
                        <div class="col-xl-12">
                            <table border="0" cellspacing="0" cellpadding="0">
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        <td class="n5">{{ $tool->getTypeCreditName($currentTotalWeekTypeCredit->type_id) }}</td>
                                        <td class="n1">{{ $tool->numberFormat($currentTotalWeekTypeCredit->montant) }}</td>
                                        <td class="n2">{{ $tool->numberFormat($lastTotalWeekTypeCredits->where('type_id', $currentTotalWeekTypeCredit->type_id)->montant) }}</td>
                                        <td class="n4">{{ $tool->numberFormat(abs($currentTotalWeekTypeCredit->montant - $lttc)) }}</td>
                                    </tr>
                                    @empty
                                    @forelse ($lastTotalWeekTypeCredits as $lastTotalWeekTypeCredit)
                                    <tr>
                                        <td class="n5">{{ $tool->getTypeCreditName($lastTotalWeekTypeCredit->type_id) }}</td>
                                        <td class="n1">{{ $tool->numberFormat(0) }}</td>
                                    ...
                                    <tr>
                                        ...
                                        <td class="n1">{{ $tool->numberFormat($currentTotalWeekTypeCredits->sum('montant')) }}</td>
                                        <td class="n2">{{ $tool->numberFormat($lastTotalWeekTypeCredits->sum('montant')) }}</td>
                                        <td class="n4">{{ $tool->numberFormat(abs($currentTotalWeekTypeCredits->sum('montant') - $lastTotalWeekTypeCredits->sum('montant'))) }}</td>
                                    </tr>
                                </tbody>
                            </table>
                    ...
                    <p>
                        ...
                        @endphp
                        @if($se < 0)
                            Cette semaine l’encours sans intérêt a baissé de  {{ $tool->numberFormat(abs($currentTotalWeekTypeCredits->sum('montant') - $lastTotalWeekTypeCredits->sum('montant'))) }}, ce qui signifie que nous avons
